...
from eval_core.models import ModelConfig, PipelineResult, BenchmarkRecord, pipeline_result_to_records
...
def render_tab_benchmark(sidebar_cfg: dict):
            ...
            for q in preguntas_to_run:
                for cfg in model_configs:
                    ...
                    try:
                        ...
                        result = pipeline.run(
                            kb, q["pregunta"], llm,
                            expected_answer=q.get("respuesta_esperada", ""),
                        )
    ...
    colors = ["#3b82f6", "#a855f7", "#ef4444", "#eab308", "#06b6d4", "#10b981"]
    ...
    for model_name, results in results_by_model.items():
        ...
        avg_r = sum(r.eval_result.relevancy.score for r in results) / n
        avg_c = sum(r.eval_result.correctness.score for r in results) / n
        avg_total = (avg_g + avg_b + avg_s + avg_d + avg_r + avg_c) / 6
        avg_lat = sum(r.latencia_s for r in results) / n

        summary_rows.append({
            "Modelo": model_name,
            "Faithfulness": f"{avg_g:.0%}",
            "Behavioral": f"{avg_b:.0%}",
            "Safety": f"{avg_s:.0%}",
            "Debate": f"{avg_d:.0%}",
            "Relevancy": f"{avg_r:.0%}",
            "Correctness": f"{avg_c:.0%}",
            "Promedio": f"{avg_total:.0%}",
            "Latencia": f"{avg_lat:.1f}s",
            "_avg": avg_total,
            "_g": avg_g, "_b": avg_b, "_s": avg_s, "_d": avg_d, "_r": avg_r, "_c": avg_c,
        })
    ...
    df_summary = pd.DataFrame(summary_rows).drop(
        columns=["_avg", "_g", "_b", "_s", "_d", "_r", "_c"]
    )
    ...
    with col_radar:
        ...
        for i, row in enumerate(summary_rows):
            vals = [row["_g"], row["_b"], row["_s"], row["_d"], row["_r"], row["_c"]]
            v2 = vals + [vals[0]]
            t2 = jnames + [jnames[0]]
    ...
    with col_bar:
        ...
        for i, row in enumerate(summary_rows):
            fig_bar.add_trace(go.Bar(
                ...
                y=[row["_g"], row["_b"], row["_s"], row["_d"], row["_r"], row["_c"]],
                marker_color=colors[i % len(colors)],
                text=[f"{s:.0%}" for s in [row["_g"], row["_b"], row["_s"], row["_d"], row["_r"], row["_c"]]],
                textposition="auto",
            ))
            ...
            with st.expander(f"{model_name}", expanded=False):
                ...
                for r, q in zip(results, questions):
                    ...
                    q_rows.append({
                        ...
                        "Faith.": f"{e.grounded.score:.0%}",
                        "Behav.": f"{e.behavioral.score:.0%}",
                        "Safety": f"{e.safety.score:.0%}",
                        "Debate": f"{e.debate.score:.0%}",
                        "Relev.": f"{e.relevancy.score:.0%}",
                        "Corr.": f"{e.correctness.score:.0%}",
                        "Prom.": f"{e.average_score:.0%}",
                    })
