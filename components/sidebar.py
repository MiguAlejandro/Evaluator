...
from eval_core.models import ModelConfig, LLMProvider
...
def render_sidebar() -> dict:
    ...
    with st.sidebar:
        ...
        # Initialize model names in session_state (only once)
        if "oai_model" not in st.session_state:
            st.session_state["oai_model"] = "gpt-4o-mini"
        if "ant_model" not in st.session_state:
            st.session_state["ant_model"] = "claude-sonnet-4-20250514"
